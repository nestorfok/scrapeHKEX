...
class Portfolio:
    ...
    def strategy(self, start_date, end_date):
        ...
        while self.cur_day != end_date:
                    ...
                    if row[col].date() == self.cur_day:
                        ...
                        if year == 2017:
                            try:
                                ...
                                self.longShort(ticker=stock_code, date=row[col].date(), score=score)
                                break
                            except:
                                pass
                        if year == 2018:
                        ...
                        if year == 2019:
                            try:
                                score = self.score_2019.loc[self.score_2019['file_name'] == stock_code]['positive'].values[0]
                                # print(stock_code, row[col].date(), year, score)
                                self.longShort(ticker=stock_code, date=row[col].date(), score=score)
                                break
                            except:
                        ...
                        if year == 2020:
                            try:
                                score = self.score_2020.loc[self.score_2020['file_name'] == stock_code]['positive'].values[0]
                                # print(stock_code, row[col].date(), year, score)
                                self.longShort(ticker=stock_code, date=row[col].date(), score=score)
                                break
                            except:
                        ...
                        if year == 2021:
                            try:
                                score = self.score_2021.loc[self.score_2021['file_name'] == stock_code]['positive'].values[0]
                                # print(stock_code, row[col].date(), year, score)
                                self.longShort(ticker=stock_code, date=row[col].date(), score=score)
                                break
                            except:
            ...
            for portfolio in self.portfolios:
                if portfolio["long"] is True and portfolio["sell_date"] == self.cur_day:
                    rm_portfolio.append(portfolio)
                # if portfolio["long"] is False and portfolio["buy_date"] == self.cur_day:
                #     rm_portfolio.append(portfolio)
            ...
            self.portfolios = [portfolio for portfolio in self.portfolios if portfolio not in rm_portfolio]
    ...
    def long(self, ticker, buy_date, buy_share_price, sell_date, sell_share_price):
        if len(self.portfolios) < self.size:
            ...
            # attach order
            buy_order = {"ticker": ticker, "long":True, "date": buy_date, "amount": buy_amount * buy_share_price, "buy": True}
            sell_order = {"ticker": ticker, "long": True, "date": sell_date, "amount": buy_amount * sell_share_price, "buy": False}
            self.buy_orders.append(buy_order)
            self.sell_orders.append(sell_order)

            # add to portfolio
            new_portfolio = {"ticker": ticker, 
                             "buy_date": buy_date, 
                             "sell_date": sell_date,
                             "amount": buy_amount, 
                             "share_price": buy_share_price,
                             "long": True}
            
            self.portfolios.append(new_portfolio)

    """def short(self, ticker, sell_date, sell_share_price, buy_date, buy_share_price):
    #     if len(self.portfolios) < self.size:
    #         divisor = (self.size - len(self.portfolios))
    #         sell_amount = int((self.balance / divisor) / sell_share_price)

    #         # attach order
    #         sell_order = {"ticker": ticker, "long": False, "date": sell_date, "amount": sell_amount * sell_share_price, "buy": False}
    #         buy_order = {"ticker": ticker, "long": False, "date": buy_date, "amount": sell_amount * buy_share_price, "buy": True}
    #         self.buy_orders.append(buy_order)
    #         self.sell_orders.append(sell_order)
            ...
            for portfolio in self.portfolios:
                if portfolio["long"] is True:
                    if portfolio["buy_date"] <= self.cur_day:
                        ...
                        stock_balance += share_price * portfolio["amount"]
                # if portfolio["long"] is False:
                #     if portfolio["sell_date"] <= self.cur_day:
                #         date, share_price = self.get_stock_data(portfolio["ticker"], self.cur_day, self.cur_day + datetime.timedelta(days=1))
                #         cash_balance += (portfolio["amount"] * portfolio["share_price"]) - (portfolio["amount"] * share_price)
    ...
    def longShort(self, ticker, date, score):
        ...
        # elif score <= 10:
        #     # short
        #     print(ticker + " score of " + str(score) + " is less than or equal to " + "10")
        #     sell_date, sell_share_price = self.get_stock_data(ticker=ticker, start=date, end= date + datetime.timedelta(days=7))
        #     buy_date, buy_share_price = self.get_stock_data(ticker=ticker, start=sell_date + datetime.timedelta(days=31), end=sell_date + datetime.timedelta(days=38))
            
        #     self.short(ticker=ticker, sell_date=sell_date, sell_share_price=sell_share_price, buy_date=buy_date, buy_share_price=buy_share_price)